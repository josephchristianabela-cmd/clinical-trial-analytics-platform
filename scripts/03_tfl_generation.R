# scripts/03_tfl_generation.R
library(haven)
library(dplyr)
library(tidyr)
library(gt) # For professional table formatting

message("--- Starting: TFL Generation ---")
...
# ADSL/ADAE already carry ARM (Description of Planned Arm), which equals
# TRT01P for this single-period study. Use it directly rather than renaming a
# treatment variable onto it - fail loudly if that assumption ever breaks,
# rather than silently guessing at a variable name.
if (!"ARM" %in% names(adsl)) stop("[ERROR] ADSL missing expected variable ARM.")
if (!"ARM" %in% names(adae)) stop("[ERROR] ADAE missing expected variable ARM.")

# FAS: randomized subjects only (excludes Screen Failure)
target_levels <- c("Placebo", "Xanomeline Low Dose", "Xanomeline High Dose")
adsl <- adsl %>% filter(ARM %in% target_levels) %>% mutate(ARM = factor(ARM, levels = target_levels))
adae <- adae %>% filter(ARM %in% target_levels) %>% mutate(ARM = factor(ARM, levels = target_levels))
...
if (!"ASTDT" %in% names(adae)) stop("[ERROR] ADAE missing expected variable ASTDT.")

serious_ae_listing <- adae %>%
  filter(AESER == "Y") %>%
  select(USUBJID, ARM, AEDECOD, AESEV, ASTDT)
...
if (!"EOSSTT" %in% names(adsl)) stop("[ERROR] ADSL missing expected variable EOSSTT.")

disposition_listing <- adsl %>%
  select(USUBJID, ARM, AGE, SEX, EOSSTT) %>%
  rename(Study_Status = EOSSTT)
